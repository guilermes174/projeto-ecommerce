  ...
  produtos.forEach((produto) => {
    ...
    tr.innerHTML = `<td class="td-produto">
                                    ...
                                    <td class="td-preco-unitario">R$ ${produto.preco
                                      .toFixed(2)
                                      .replace(".", ",")}</td>
                                    <td class="td-quantidade"><input type="number" class="produto-quantidade" data-id="${
                                      produto.id
...
corpoTabela.addEventListener("input", (evento) => {
  //passo 2 - atualizar o valor total do produto
  if (evento.target.classList.contains("produto-quantidade")) {
    const produtos = obterProdutosDoCarrinho();
...
function atualizarValorTotalDoCarrinho() {
  ...
  document.querySelector("#total-carrinho").textContent = `R$ ${total
    .toFixed(2)
    .replace(".", ",")}`;
}
